...
- Windows 升级改为版本目录并存和稳定启动器切换，不再要求 npm 重命名被占用的旧全局包目录。
- 新增 `feishu-codex-bridge upgrade [版本]`：下载新包、验证、切换并等待服务 ready，失败时自动恢复升级前入口。
- 旧 npm 包目录清理失败只记录 `pending-cleanup.json` 并延迟处理，不再阻塞新版本启用。
- 自动启动任务优先指向稳定启动器，后续升级无需重新注册计划任务。
