...
## P0-20260730-08：npm 全局目录被外部进程锁定导致正式版无法升级

- 状态：已修复、发布并通过 `0.2.2` 本机 UAT
- 发现版本：`0.2.1`
- 修复版本：`0.2.2`
- 环境：Windows、Node.js 24、npm 11

停止 Bridge 主进程及已记录子进程后，npm 仍连续三次无法将 scoped 包目录重命名为退役目录。Restart Manager 未发现包内 43 个文件被占用，目录级占用者无法通过内置接口确定；继续延长等待或重复原地安装不能可靠解决。

`0.2.2` 改用独立版本目录和稳定启动器。新包验证通过后切换 `current.json`，启动失败自动恢复旧入口；旧 npm 包目录被占用时写入延迟清理记录，不再阻塞升级。安装器同时修正 scoped 包目录提示缺少 `@tianniansz` 的问题。
